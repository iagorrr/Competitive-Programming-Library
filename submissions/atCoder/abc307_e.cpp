...
mint<MOD> pows[MAXN + 1], pows2[MAXN + 1];
/*
 * ideas:
 *  -  inclusao exclusao conta os que 2 consecutivos - os de
 * 3 + os de 4... for i in [1..N]:
 *  - so exclusao mesmo considera caso em que é i
 * consecutivos
 *    - M = 2 é caso de borda !
 *
 *
...
auto run() {
  ...
  // if (M == 1) return 0ll;
  // if (M == 2) return 2ll;
  // if (N == 2) return (M * (M - 1ll)) % MOD;
  // if (N == 3) return M * (M - 1) * (M - 2) % MOD;

  mint initial(1), not_initial(0);
  dbg(initial, not_initial);
  for (int i = 1; i <= N - 1; i++) {
    auto t1 = initial, t2 = not_initial;

    initial = t2;
    not_initial = (t2 * mint(M - 2)) + (t1 * mint(M - 1));

    dbg(initial, not_initial);
  }
  mint ans = not_initial * mint(M);
