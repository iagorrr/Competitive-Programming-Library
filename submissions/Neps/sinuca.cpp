...
int main(void){
    ...
    LOOP(i, 0, N){
        cin >> temp;
        bolasAtual.push_back(temp);
    }

    while(bolasAtual.size() != 1){
        
        vll bolasTemp;
        
        if(bolasAtual.size() == 2){
            if(bolasAtual[0] != bolasAtual[1]) bolasTemp.push_back(-1);
            else bolasTemp.push_back(1);
        }
        else{
            LOOP(i, 1, bolasAtual.size()){
                if(bolasAtual[i] != bolasAtual[i-1]) bolasTemp.push_back(-1);
                else bolasTemp.push_back(1);
            }
        }
        bolasAtual = bolasTemp;
        // LOOP(i, 0, bolasAtual.size()){
        //     cout << bolasAtual[i] << " ";
        // }
        // cout << endl;
    }
    if(bolasAtual[0] <= 0) cout << "branca" << endl;
    else cout << "preta" << endl;
	return 0;
}
/*====================================================================================================*/
