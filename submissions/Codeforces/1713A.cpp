...
lli solve(vector <pair<lli, lli>> b){
    ...
    lli ans = 0;
    LOOP(i, 1, b.size()){
        ans+= max(b[i].first - b[i-1].first, b[i-1].first - b[i].first) + max(b[i].second - b[i-1].second, b[i-1].second - b[i].second);
    }
    ans += max(b[0].first, -b[0].first)+ max(b[0].second, -b[0].second);
    ans += max(b[b.size()-1].first, -b[b.size()-1].first)+ max(b[b.size()-1].second, -b[b.size()-1].second);
    return ans;
}
