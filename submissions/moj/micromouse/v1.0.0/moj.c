# 0 "main.c"
# 1 "/home/iagorrr/code/micromouse/bobinho//"
# 0 "<built-in>"
# 0 "<command-line>"
...
# 1 "/usr/lib/gcc/x86_64-pc-linux-gnu/13.1.1/include/stdbool.h" 1 3 4
# 4 "map.c" 2
...
extern __inline __attribute__((__gnu_inline__)) int vprintf(
...
extern __inline __attribute__((__gnu_inline__)) int fgetc_unlocked(FILE *__fp) {
        return (__builtin_expect(((__fp)->_IO_read_ptr >= (__fp)->_IO_read_end),
                                 0)
                        ? __uflow(__fp)
...
extern __inline __attribute__((__gnu_inline__)) int getchar_unlocked(void) {
        return (__builtin_expect(
                        ((stdin)->_IO_read_ptr >= (stdin)->_IO_read_end), 0)
                        ? __uflow(stdin)
...
extern __inline __attribute__((__gnu_inline__)) int fputc_unlocked(
        ...
        return (__builtin_expect(((__stream)->_IO_write_ptr >=
                        ...
                        : (unsigned char)(*(__stream)->_IO_write_ptr++ =
                                                  (__c)));
}

extern __inline __attribute__((__gnu_inline__)) int putc_unlocked(
        int __c, FILE *__stream) {
        return (__builtin_expect(((__stream)->_IO_write_ptr >=
                                  (__stream)->_IO_write_end),
                                 0)
...
extern __inline __attribute__((__gnu_inline__)) int putchar_unlocked(int __c) {
        return (__builtin_expect(
                        ((stdout)->_IO_write_ptr >= (stdout)->_IO_write_end), 0)
...
int direction = 0;
coord directions_vectors[4] = {(coord){-1, 0}, (coord){0, 1}, (coord){1, 0},
                               (coord){0, -1}};

map_cell __MAP[(unsigned int)5495 * 2][(unsigned int)5495 * 2];

coord translate(coord *c) {
        return (coord){c->i + (unsigned int)5495, c->j + (unsigned int)5495};
}
...
# 4 "explore.c" 2
# 12 "explore.c"
void mark_movable(coord c, int direction,
# 12 "explore.c" 3 4
                  _Bool
# 12 "explore.c"
                          possible);
...
# 25 "explore.c" 3 4
        _Bool
# 25 "explore.c"
                response = (__MAP[t.i][t.j].dir & 0b11110000) == 0b11110000;
...
# 55 "explore.c"
        ...
        mark_movable(current_cord, direction, r != 1);
...
# 111 "explore.c"
        ...
        c = translate(&c);
        __MAP[c.i][c.j].dir |= (1 << (direction + 4));
        if (possible)
                __MAP[c.i][c.j].dir |= (1 << (direction));
        else
                __MAP[c.i][c.j].dir &= ~(1 << (direction));
}
...
# 140 "explore.c"
        ...
        coord tmp;
        while (tmp = simulatePos(current_cord, direction),

               tmp.i != parent.i || tmp.j != parent.j) {
                rotateRight();
        }
        move();
...
# 1 "/usr/lib/gcc/x86_64-pc-linux-gnu/13.1.1/include/stddef.h" 1 3 4
# 329 "/usr/lib/gcc/x86_64-pc-linux-gnu/13.1.1/include/stddef.h" 3 4
...
extern __inline __attribute__((__gnu_inline__)) void *bsearch(
...
# 37 "queue.c"
                        ,
                        "QUEUE-PUSH-BACK: faltou memória\n");
                fflush(
# 38 "queue.c" 3 4
                        stderr
# 38 "queue.c"
...
# 16 "bfs.c"
                ,
                "to reconstruindo de [%d, %d] para [%d, %d]\n", s->i, s->j,
                e->i, e->j);
        fflush(
# 17 "bfs.c" 3 4
                stderr
...
# 24 "main.c"
                ,
                "fazendo caminho da saida pro inicio : \n");
...
# 35 "main.c"
                ,
                "terminei caminho da saída pro inicio : \n");
