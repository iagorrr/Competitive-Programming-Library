...
int move() {
        ...
        if (r != CRASHED) {  // atualiza as coordenadas
                // fprintf(stderr, "movi do {%d, %d}", current_cord.i,
                // current_cord.j);
                current_cord = simulatePos(current_cord, direction);
                // fprintf(stderr, "pro {%d, %d}\n", current_cord.i,
                // current_cord.j);
        }
...
bool dfs(coord parent, int m) {
        while (!checkFoda()) {
                ...
                if (!isKnown(current_cord, direction)) {
                        ...
                        if (code == WALKED) {
                                // TODO : ver essa porr adessep ai
                                // cord pai = simulatePos(current_cord,
                                // opposite(direction)); fprintf(stderr, "vim de
                                // %d %d e to no %d %d\n", previous.i,
                                // previous.j, // current_cord.i,
                                // current_cord.j);
                                if (dfs(previous, m + 1)) {
                                        fprintf(stderr,
        ...
        // logger("sai do checkfoda", m);
        coord tmp;
        while (tmp = simulatePos(current_cord, direction),
               // printf("simulateI = %d; simulateJ = %d\n pi : %d pj: %d\n",
               // tmp.i, tmp.j, parent.i, parent.j),
               tmp.i != parent.i || tmp.j != parent.j) {
                // logger("to a rodar pro pai", m);
                // TODO: rodar pra uma direção melhor
                rotateRight();
        }
        move();
        // logger("movi?", m);
